            ...
            <div className={`flex flex-col h-screen p-4 w-1/8 bg-gray-100 dark:bg-darkBlack ${showMenu ? 'slide-in' : 'slide-out'}`}>
              <div className='flex flex-col h-full justify-between'>
                <div className='grid gap-2 overflow-y-auto'>
                  {pages.map(({ name }) => (
                    <Link
                      key={name}
                      to={`/admin/home/${name}`}
                      className={`cshadow p-2 px-4 mx-2 rounded-3xl font-bold text-lg hover:scale-110 transition-all cursor-pointer dark:bg-lightBlack 
                ...
                <div className='grid grid-rows-2'>
                  <ThemeToggle />
                  <div
                    className="cshadow flex justify-start items-center px-4 my-2 rounded-3xl font-bold text-lg hover:scale-110 transition-all duration-200 cursor-pointer dark:bg-lightBlack"
                    onClick={onLogout}
                  >
                    Logout
                  </div>
